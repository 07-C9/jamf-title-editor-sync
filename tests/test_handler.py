...
class TestUpdateNotice(unittest.TestCase):
    ...
    def _results(self, updated):
        base = [{"app": "utiluti", "status": "current", "version": "1.5"}]
        return base + [
            {"app": a, "status": "updated", "from": f, "to": t} for a, f, t in updated
        ]

    def test_single_update_names_the_app_and_new_version_in_the_subject(self):
        handler = _reload_handler()
        subject, body = handler._build_update_notice(
            self._results([("Google Chrome", "150.0.7871.182", "150.0.7871.190")]), "req-1"
        )
        self.assertIn("Google Chrome", subject)
        self.assertIn("150.0.7871.190", subject)
        self.assertIn("150.0.7871.182 -> 150.0.7871.190", body)

    def test_multiple_updates_are_all_listed(self):
        handler = _reload_handler()
        subject, body = handler._build_update_notice(
            self._results([
                ("Google Chrome", "150.0.7871.182", "150.0.7871.190"),
                ("DRC INSIGHT", "17.0.0", "17.1.0"),
            ]), "req-1"
        )
        self.assertIn("2", subject)
        self.assertIn("Google Chrome: 150.0.7871.182 -> 150.0.7871.190", body)
        self.assertIn("DRC INSIGHT: 17.0.0 -> 17.1.0", body)

    def test_apps_that_did_not_change_are_not_listed(self):
        handler = _reload_handler()
        _, body = handler._build_update_notice(
            self._results([("DRC INSIGHT", "17.0.0", "17.1.0")]), "req-1"
        )
        self.assertNotIn("utiluti", body)

    def test_subject_stays_within_the_sns_limit(self):
        handler = _reload_handler()
        subject, _ = handler._build_update_notice(
            self._results([("A" * 200, "1.0", "2.0")]), "req-1"
        )
        self.assertLessEqual(len(subject), 100)


class TestPublishUpdateNotice(unittest.TestCase):
    ...
    def test_publishes_when_a_version_was_pushed(self):
        results = [{"app": "DRC INSIGHT", "status": "updated", "from": "17.0.0", "to": "17.1.0"}]
        with patch("handler.boto3.client", return_value=self.mock_sns):
            self.handler._publish_update_notice(results, "req-1")
        self.mock_sns.publish.assert_called_once()
        self.assertIn("17.1.0", self.mock_sns.publish.call_args.kwargs["Message"])

    def test_silent_when_nothing_was_pushed(self):
        results = [{"app": "DRC INSIGHT", "status": "current", "version": "17.0.0"}]
        with patch("handler.boto3.client", return_value=self.mock_sns):
            self.handler._publish_update_notice(results, "req-1")
        self.mock_sns.publish.assert_not_called()

    def test_sns_failure_does_not_break_the_run(self):
        """A notification is not worth failing a successful sync over."""
        self.mock_sns.publish.side_effect = RuntimeError("SNS down")
        results = [{"app": "DRC INSIGHT", "status": "updated", "from": "17.0.0", "to": "17.1.0"}]
        with patch("handler.boto3.client", return_value=self.mock_sns):
            self.handler._publish_update_notice(results, "req-1")
